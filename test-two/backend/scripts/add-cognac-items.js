...
async function addCognacItems() {
  try {
    ...
    console.log('Cognac category found:', cognacCategory.name);

    // Create subcategory for cognac
    const [cognacSubCategory] = await db.SubCategory.findOrCreate({
      where: { name: 'All Cognac', categoryId: cognacCategory.id },
      defaults: { description: 'All types of cognac', categoryId: cognacCategory.id }
    });
    console.log('Cognac subcategory found/created:', cognacSubCategory.name);
    ...
    console.log(`\nCompleted adding cognac items:\n- Added: ${addedCount}\n- Skipped: ${skippedCount}\n- Total processed: ${cognacProducts.length}`);
