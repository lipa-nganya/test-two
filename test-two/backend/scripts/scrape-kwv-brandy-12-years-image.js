...
async function scrapeKWVBrandy12YearsImage() {
  try {
    console.log('Scraping KWV brandy 12 years image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/kwv-brandy-12-years';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('kwv') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'kwv-brandy-12-years.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%KWV brandy 12 years%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  KWV brandy 12 years not found in database.');
    }
...
scrapeKWVBrandy12YearsImage()
  .then(() => {
    console.log('KWV brandy 12 years image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
