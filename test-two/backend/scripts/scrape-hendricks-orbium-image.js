...
async function scrapeHendricksOrbiumImage() {
  try {
    console.log('Scraping Hendricks Orbium image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/hendricks-orbium';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('hendricks') || src.includes('orbium') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'hendricks-orbium.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Hendricks Orbium%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Hendricks Orbium not found in database.');
    }
...
scrapeHendricksOrbiumImage()
  .then(() => {
    console.log('Hendricks Orbium image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
