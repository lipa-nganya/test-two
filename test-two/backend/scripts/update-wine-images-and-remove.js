...
async function updateWineImagesAndRemove() {
  try {
    console.log('🖼️  Updating wine images and removing items...\n');
    ...
    // Remove "Dial A Drink Kenya" from wine category
    console.log('Removing items...\n');
    const itemToRemove = await db.Drink.findOne({
      where: { 
        name: { [db.Sequelize.Op.iLike]: 'Dial A Drink Kenya' },
    ...
    if (itemToRemove) {
      ...
      console.log(`✅ Removed: Dial A Drink Kenya`);
    } else {
      console.log(`⚠️  Not found: Dial A Drink Kenya`);
    }

    // Items to update images
    const itemsToUpdate = [
      { name: 'Don Santiago Sweet Red Wine', url: 'https://www.dialadrinkkenya.com/search/don-santiago-sweet-red-wine' },
      { name: 'Fragolino DuchesaLia', url: 'https://www.dialadrinkkenya.com/search/fragolino-duchesalia' },
      { name: 'Mateus Rose', url: 'https://www.dialadrinkkenya.com/search/mateus-rose' }
    ];
    ...
    console.log(`\n✅ Completed!`);
    console.log(`- Removed: 1 item (Dial A Drink Kenya)`);
