...
async function scrapeGinSocietyOriginalGinImage() {
  try {
    console.log('Scraping Gin Society Original Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/gin-society-original-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('gin-society') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'gin-society-original-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Gin Society Original Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Gin Society Original Gin not found in database.');
    }
...
scrapeGinSocietyOriginalGinImage()
  .then(() => {
    console.log('Gin Society Original Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
