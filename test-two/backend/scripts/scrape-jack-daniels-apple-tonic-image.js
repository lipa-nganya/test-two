...
async function scrapeJackDanielsAppleTonicImage() {
  try {
    console.log('Scraping Jack Daniel\'s Apple Tonic Beer image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/jack-daniel-s-apple-tonic-beer';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('jack') || src.includes('daniel') || src.includes('apple') || src.includes('tonic') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'jack-daniels-apple-tonic-beer.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Jack Daniel\'s Apple Tonic%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Jack Daniel\'s Apple Tonic Beer not found in database.');
    }
...
scrapeJackDanielsAppleTonicImage()
  .then(() => {
    console.log('Jack Daniel\'s Apple Tonic Beer image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
