...
async function scrapeDonMontegoVSOPImage() {
  try {
    console.log('Scraping Don Montego VSOP image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/don-montego-vsop';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('don') || src.includes('montego') || src.includes('vsop') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'don-montego-vsop.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Don Montego VSOP%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Don Montego VSOP not found in database.');
    }
...
scrapeDonMontegoVSOPImage()
  .then(() => {
    console.log('Don Montego VSOP image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
