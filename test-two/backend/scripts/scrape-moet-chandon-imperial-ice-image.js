...
async function scrapeMoetChandonImperialIceImage() {
  try {
    console.log('Scraping Moet & Chandon Imperial Ice image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/moet-chandon-imperial-ice';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('moet') || src.includes('chandon') || src.includes('imperial') || src.includes('ice') || src.includes('champagne') || src.includes('product'))) {
          imageUrl = src;
    ...
    // Save the image
    const filename = 'moet-chandon-imperial-ice.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Moet%Chandon%Imperial%Ice%'
        }
      }
    });
    ...
    console.error('Error scraping Moet & Chandon Imperial Ice image:', error.message);
    throw error;
  } finally {
...
// Run the scraper
scrapeMoetChandonImperialIceImage()
  .then(() => {
    console.log('Moet & Chandon Imperial Ice image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Failed to scrape Moet & Chandon Imperial Ice image:', error);
    process.exit(1);
  });