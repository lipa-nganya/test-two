...
async function scrapeCourvoisierVSImage() {
  try {
    console.log('Scraping Courvoisier VS image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/courvoisier-vs-cognac-1litre';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('courvoisier') || src.includes('cognac') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'courvoisier-vs.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Courvoisier VS%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Courvoisier VS not found in database.');
    }
...
scrapeCourvoisierVSImage()
  .then(() => {
    console.log('Courvoisier VS image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
