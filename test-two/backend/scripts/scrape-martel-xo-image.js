...
async function scrapeMartelXOImage() {
  try {
    console.log('Scraping Martel XO image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/martel-xo';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('martel') || src.includes('martell') || src.includes('cognac') || src.includes('product'))) {
          imageUrl = src;
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Martel XO%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Martel XO not found in database.');
    }
...
scrapeMartelXOImage()
  .then(() => {
    console.log('Martel XO image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
