...
async function scrapeTuskerCiderImage() {
  try {
    console.log('Scraping Tusker Cider image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/tusker-cider';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tusker') || src.includes('cider') || src.includes('beer') || src.includes('product'))) {
          imageUrl = src;
    ...
    // Save the image
    const filename = 'tusker-cider.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Tusker Cider%'
        }
      }
    });
    ...
    console.error('Error scraping Tusker Cider image:', error.message);
    throw error;
  } finally {
...
// Run the scraper
scrapeTuskerCiderImage()
  .then(() => {
    console.log('Tusker Cider image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Failed to scrape Tusker Cider image:', error);
    process.exit(1);
  });
