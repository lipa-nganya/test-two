...
async function scrapeGrandMarnierRaspberryPeachImage() {
  try {
    console.log('Scraping Grand Marnier Raspberry Peach image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/grand-marnier-raspberry-peach';
    ...
    const filename = 'grand-marnier-raspberry-peach.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Grand Marnier Raspberry Peach%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Grand Marnier Raspberry Peach not found in database.');
    }
...
scrapeGrandMarnierRaspberryPeachImage()
  .then(() => {
    console.log('Grand Marnier Raspberry Peach image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
