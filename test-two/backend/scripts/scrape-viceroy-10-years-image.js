...
async function scrapeViceroy10YearsImage() {
  try {
    console.log('Scraping Viceroy 10 Years image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/viceroy-10-years';
    ...
    const filename = 'viceroy-10-years.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Viceroy 10 Years%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Viceroy 10 Years not found in database.');
    }
...
scrapeViceroy10YearsImage()
  .then(() => {
    console.log('Viceroy 10 Years image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
