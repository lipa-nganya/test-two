...
async function scrapeHuntersGoldCiderImage() {
  try {
    console.log('Scraping Hunter\'s Gold Cider image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/hunter-s-gold-cider';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('hunter') || src.includes('gold') || src.includes('cider') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'hunters-gold-cider.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Hunter\'s Gold Cider%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Hunter\'s Gold Cider not found in database.');
    }
...
scrapeHuntersGoldCiderImage()
  .then(() => {
    console.log('Hunter\'s Gold Cider image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
