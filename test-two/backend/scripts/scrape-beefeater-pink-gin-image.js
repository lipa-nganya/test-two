...
async function scrapeBeefeaterPinkGinImage() {
  try {
    console.log('Scraping Beefeater Pink Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/beefeater-pink-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('beefeater') || src.includes('pink') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'beefeater-pink-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Beefeater Pink Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Beefeater Pink Gin not found in database.');
    }
...
scrapeBeefeaterPinkGinImage()
  .then(() => {
    console.log('Beefeater Pink Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
