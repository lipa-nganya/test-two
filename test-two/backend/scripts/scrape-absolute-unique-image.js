...
async function scrapeAbsoluteUniqueImage() {
  try {
    console.log('Scraping Absolute Unique image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/absolute-unique';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('absolute') || src.includes('unique') || src.includes('vodka') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'absolute-unique.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Absolute Unique%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Absolute Unique not found in database.');
    }
...
scrapeAbsoluteUniqueImage()
  .then(() => {
    console.log('Absolute Unique image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
