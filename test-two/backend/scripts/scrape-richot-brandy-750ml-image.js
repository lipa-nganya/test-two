...
async function scrapeRichotBrandy750mlImage() {
  try {
    console.log('Scraping Richot Brandy 750ml image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/richot-brandy-750ml';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('richot') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'richot-brandy-750ml.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Richot Brandy 750ml%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Richot Brandy 750ml not found in database.');
    }
...
scrapeRichotBrandy750mlImage()
  .then(() => {
    console.log('Richot Brandy 750ml image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
