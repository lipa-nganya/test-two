...
async function scrapeHopsmithLagerGlutenFreeImage() {
  try {
    console.log('Scraping Hopsmith Lager Gluten Free image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/hopsmith-lager-gluten-free';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('hopsmith') || src.includes('lager') || src.includes('gluten') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'hopsmith-lager-gluten-free.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Hopsmith Lager Gluten Free%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Hopsmith Lager Gluten Free not found in database.');
    }
...
scrapeHopsmithLagerGlutenFreeImage()
  .then(() => {
    console.log('Hopsmith Lager Gluten Free image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
