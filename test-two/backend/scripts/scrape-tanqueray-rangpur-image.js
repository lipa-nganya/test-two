...
async function scrapeTanquerayRangpurImage() {
  try {
    console.log('Scraping Tanqueray Rangpur image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/tanqueray-rangpur-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tanqueray') || src.includes('rangpur') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'tanqueray-rangpur.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Tanqueray Rangpur%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Tanqueray Rangpur not found in database.');
    }
...
scrapeTanquerayRangpurImage()
  .then(() => {
    console.log('Tanqueray Rangpur image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
