...
async function scrapeHendricksAmazoniaImage() {
  try {
    console.log('Scraping Hendricks Amazonia image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/hendricks-amazonia';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('hendricks') || src.includes('amazonia') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'hendricks-amazonia.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Hendricks Amazonia%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Hendricks Amazonia not found in database.');
    }
...
scrapeHendricksAmazoniaImage()
  .then(() => {
    console.log('Hendricks Amazonia image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
