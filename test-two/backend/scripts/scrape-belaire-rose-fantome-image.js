...
async function scrapeBelaireRoseFantomeImage() {
  try {
    console.log('Scraping Belaire Rose Fantome image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/belaire-rose-fantome';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('belaire') || src.includes('rose') || src.includes('champagne') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'belaire-rose-fantome.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Belaire Rose Fantome%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Belaire Rose Fantome not found in database.');
    }
...
scrapeBelaireRoseFantomeImage()
  .then(() => {
    console.log('Belaire Rose Fantome image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
