...
async function scrapeOettingerPils47Image() {
  try {
    console.log('Scraping Oettinger Pils 4.7% image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/oettinger-pils-4-7';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('oettinger') || src.includes('pils') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'oettinger-pils-4.7.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Oettinger Pils 4.7%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Oettinger Pils 4.7% not found in database.');
    }
...
scrapeOettingerPils47Image()
  .then(() => {
    console.log('Oettinger Pils 4.7% image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
