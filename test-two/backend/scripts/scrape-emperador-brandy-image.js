...
async function scrapeEmperadorBrandyImage() {
  try {
    console.log('Scraping Emperador Brandy image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search?query=Emperador+Brandy';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('emperador') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'emperador-brandy.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Emperador Brandy%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Emperador Brandy not found in database.');
    }
...
scrapeEmperadorBrandyImage()
  .then(() => {
    console.log('Emperador Brandy image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
