...
async function scrapeGrandMarnierImage() {
  try {
    console.log('Scraping Grand Marnier image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/grand-marnier';
    ...
    const filename = 'grand-marnier.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Grand Marnier%'
        },
        name: {
          [db.Sequelize.Op.notILike]: '%Raspberry%'
        },
        name: {
          [db.Sequelize.Op.notILike]: '%Nuit%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Grand Marnier not found in database.');
    }
...
scrapeGrandMarnierImage()
  .then(() => {
    console.log('Grand Marnier image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
