...
async function scrapeTanquerayMalaccaGinImage() {
  try {
    console.log('Scraping Tanqueray Malacca Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/tanqueray-malacca-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tanqueray') || src.includes('malacca') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'tanqueray-malacca-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Tanqueray Malacca Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Tanqueray Malacca Gin not found in database.');
    }
...
scrapeTanquerayMalaccaGinImage()
  .then(() => {
    console.log('Tanqueray Malacca Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
