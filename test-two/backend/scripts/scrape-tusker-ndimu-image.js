...
async function scrapeTuskerNdimuImage() {
  try {
    console.log('Scraping Tusker Ndimu image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/tusker-ndimu';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tusker') || src.includes('ndimu') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'tusker-ndimu.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Tusker Ndimu%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Tusker Ndimu not found in database.');
    }
...
scrapeTuskerNdimuImage()
  .then(() => {
    console.log('Tusker Ndimu image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
