...
async function scrapeBeefeaterClearGinImage() {
  try {
    console.log('Scraping Beefeater Clear Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/beefeater-clear-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('beefeater') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'beefeater-clear-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Beefeater Clear Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Beefeater Clear Gin not found in database.');
    }
...
scrapeBeefeaterClearGinImage()
  .then(() => {
    console.log('Beefeater Clear Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
