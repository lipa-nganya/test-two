...
async function scrapeGunpowderIrishGinImage() {
  try {
    console.log('Scraping Gunpowder Irish Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/gunpowder-irish-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('gunpowder') || src.includes('irish') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'gunpowder-irish-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Gunpowder Irish Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Gunpowder Irish Gin not found in database.');
    }
...
scrapeGunpowderIrishGinImage()
  .then(() => {
    console.log('Gunpowder Irish Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
