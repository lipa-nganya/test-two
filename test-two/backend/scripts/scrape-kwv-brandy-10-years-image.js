...
async function scrapeKWVBrandy10YearsImage() {
  try {
    console.log('Scraping KWV Brandy 10 years image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/kwv-brandy-10-years';
    ...
    const filename = 'kwv-brandy-10-years.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%KWV Brandy 10 years%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  KWV Brandy 10 years not found in database.');
    }
...
scrapeKWVBrandy10YearsImage()
  .then(() => {
    console.log('KWV Brandy 10 years image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
