...
async function scrapeJackDanielsHoneyLemonadeImage() {
  try {
    console.log('Scraping Jack Daniel\'s Honey Lemonade image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/jack-daniel-s-honey-lemonade';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('jack') || src.includes('daniel') || src.includes('honey') || src.includes('lemonade') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'jack-daniels-honey-lemonade.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Jack Daniel\'s Honey Lemonade%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Jack Daniel\'s Honey Lemonade not found in database.');
    }
...
scrapeJackDanielsHoneyLemonadeImage()
  .then(() => {
    console.log('Jack Daniel\'s Honey Lemonade image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
