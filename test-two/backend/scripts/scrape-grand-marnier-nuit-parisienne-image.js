...
async function scrapeGrandMarnierNuitParisienneImage() {
  try {
    console.log('Scraping Grand Marnier Nuit Parisienne image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/grand-marnier-nuit-parisienne';
    ...
    const selectors = [
      'img[src*="grand-marnier"]',
      'img[alt*="Grand Marnier"]',
      'img[alt*="Nuit"]',
      '.product-image img',
      '.product-detail img',
      '.product-img img',
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('grand') || src.includes('marnier') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'grand-marnier-nuit-parisienne.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Grand Marnier Nuit Parisienne%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Grand Marnier Nuit Parisienne not found in database.');
    }
...
scrapeGrandMarnierNuitParisienneImage()
  .then(() => {
    console.log('Grand Marnier Nuit Parisienne image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
