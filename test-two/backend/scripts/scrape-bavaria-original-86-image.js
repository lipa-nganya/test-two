...
async function scrapeBavariaOriginal86Image() {
  try {
    console.log('Scraping Bavaria Original 8.6% image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/bavaria-original-8-6';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('bavaria') || src.includes('original') || src.includes('8.6') || src.includes('beer') || src.includes('product'))) {
          imageUrl = src;
    ...
    // Save the image
    const filename = 'bavaria-original-8.6.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Bavaria Original 8.6%'
        }
      }
    });
    ...
    console.error('Error scraping Bavaria Original 8.6% image:', error.message);
    throw error;
  } finally {
...
// Run the scraper
scrapeBavariaOriginal86Image()
  .then(() => {
    console.log('Bavaria Original 8.6% image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Failed to scrape Bavaria Original 8.6% image:', error);
    process.exit(1);
  });
