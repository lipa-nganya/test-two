...
async function scrapeRaynalVSOPBrandyImage() {
  try {
    console.log('Scraping Raynal Vsop Brandy image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/raynal-vsop-brandy';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('raynal') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'raynal-vsop-brandy.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Raynal Vsop Brandy%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Raynal Vsop Brandy not found in database.');
    }
...
scrapeRaynalVSOPBrandyImage()
  .then(() => {
    console.log('Raynal Vsop Brandy image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
