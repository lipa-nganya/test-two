...
async function scrapeTuskerLagerImage() {
  try {
    console.log('Scraping Tusker Lager image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/tusker-%20beer';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tusker') || src.includes('lager') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'tusker-lager.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Tusker Lager%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Tusker Lager not found in database.');
    }
...
scrapeTuskerLagerImage()
  .then(() => {
    console.log('Tusker Lager image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
