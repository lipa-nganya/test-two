...
async function getImageFromPage(pageUrl, drinkName, isFirstImage = false) {
  try {
    ...
    // For first image on the left (Woodford Reserve), get the first product image
    if (isFirstImage) {
      // Method 1: Look for the first product image in product cards/containers
      $('[class*="product"], [class*="card"], .mobile-product-card').each((i, el) => {
        if (i === 0) { // First product
          const $img = $(el).find('img[src*="cloudinary"]').not('[src*="logo"]').first();
      ...
      // Method 2: Look for first image with product context (more lenient)
      if (!imageUrl) {
        $('img[src*="cloudinary"]').not('[src*="logo"]').each((i, el) => {
          if (i === 0) {
      ...
      // Method 3: Get first product image from JSON-LD
      if (!imageUrl) {
        $('script[class="json"][type="application/ld+json"]').each((i, el) => {
          try {
            ...
            // Check if name matches the drink we're looking for (first word match)
            if (data.name && i === 0) { // First product JSON-LD
              if (data.image) {
                let url = null;
      ...
      // Standard extraction: Look for product image in JSON-LD
      $('script[type="application/ld+json"]').each((i, el) => {
        try {
          ...
          if (data.name && data.name.toLowerCase().includes(drinkName.toLowerCase().split(' ')[0])) {
            if (data.image) {
              let url = null;
              if (typeof data.image === 'string') {
                url = data.image;
...
async function updateWhiskyImages() {
  try {
    console.log('🖼️  Updating whisky images...\n');
    ...
    const itemsToUpdate = [
      { name: 'Lagavulin 12 Years', url: 'https://www.dialadrinkkenya.com/search/lagavulin-12-years', isFirstImage: false },
      { name: 'Monkey Shoulder', url: 'https://www.dialadrinkkenya.com/search/monkey-shoulder', isFirstImage: false },
      { name: 'Proper No.Twelve Whisky', url: 'https://www.dialadrinkkenya.com/search/proper-no-twelve-whisky', isFirstImage: false },
      { name: 'Suntory Kakubin', url: 'https://www.dialadrinkkenya.com/search/suntory-kakubin', isFirstImage: false },
      { name: 'Whytehall Fire', url: 'https://www.dialadrinkkenya.com/search/whytehall-fire', isFirstImage: false },
      { name: 'Woodford Reserve', url: 'https://www.dialadrinkkenya.com/search/woodford-reserve', isFirstImage: true }
    ];
    ...
    for (let i = 0; i < itemsToUpdate.length; i++) {
      const { name, url, isFirstImage } = itemsToUpdate[i];
      ...
      const imageUrl = await getImageFromPage(url, name, isFirstImage);
    ...
    console.error('Error updating whisky images:', error);
  } finally {
