...
async function scrapeThreeBarrelsVSOPImage() {
  try {
    console.log('Scraping Three Barrels VSOP image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/three-barrels-vsop';
    ...
    const selectors = [
      'img[src*="three-barrels"]',
      'img[src*="three.barrels"]',
      'img[alt*="Three Barrels"]',
      'img[alt*="Three-Barrels"]',
      'img[alt*="VSOP"]',
      '.product-image img',
      '.product-detail img',
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('three') || src.includes('barrels') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'three-barrels-vsop.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Three Barrels VSOP%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Three Barrels VSOP not found in database.');
    }
...
scrapeThreeBarrelsVSOPImage()
  .then(() => {
    console.log('Three Barrels VSOP image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
