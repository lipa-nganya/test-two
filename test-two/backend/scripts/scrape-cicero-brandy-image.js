...
async function scrapeCiceroBrandyImage() {
  try {
    console.log('Scraping Cicero Brandy image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/cicero-brandy';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('cicero') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'cicero-brandy.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Cicero Brandy%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Cicero Brandy not found in database.');
    }
...
scrapeCiceroBrandyImage()
  .then(() => {
    console.log('Cicero Brandy image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
