...
async function scrapeBudweiserBeerCanImage() {
  try {
    console.log('Scraping Budweiser Beer Can image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/Budweiser%20Beer%20Can';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('budweiser') || src.includes('beer') || src.includes('product'))) {
          imageUrl = src;
    ...
    // Save the image
    const filename = 'budweiser-beer-can.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Budweiser%'
        }
      }
    });
    ...
    console.error('Error scraping Budweiser Beer Can image:', error.message);
    throw error;
  } finally {
...
// Run the scraper
scrapeBudweiserBeerCanImage()
  .then(() => {
    console.log('Budweiser Beer Can image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Failed to scrape Budweiser Beer Can image:', error);
    process.exit(1);
  });
