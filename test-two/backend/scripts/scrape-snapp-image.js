...
async function scrapeSnappImage() {
  try {
    console.log('Scraping Snapp image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search?query=snapp';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('snapp') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    if (drink) {
      ...
      console.log('⚠️  Snapp not found in database.');
    }
...
scrapeSnappImage()
  .then(() => {
    console.log('Snapp image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
