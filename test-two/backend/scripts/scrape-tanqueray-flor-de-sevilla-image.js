...
async function scrapeTanquerayFlorDeSevillaImage() {
  try {
    console.log('Scraping Tanqueray Flor de Sevilla image from Dial a Drink Kenya...');
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tanqueray') || src.includes('sevilla') || src.includes('flor') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'tanqueray-flor-de-sevilla.jpg';
...
scrapeTanquerayFlorDeSevillaImage()
  .then(() => {
    console.log('Tanqueray Flor de Sevilla image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
