...
async function replaceHeinekenLightImage() {
  try {
    console.log('Scraping Heineken Zero image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/heineken-zero-alcohol';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('heineken') || src.includes('zero') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'heineken-light.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Heineken Light%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Heineken Light not found in database.');
    }
...
replaceHeinekenLightImage()
  .then(() => {
    console.log('Heineken Light image replacement completed successfully');
    process.exit(0);
  })
  .catch((error) => {
