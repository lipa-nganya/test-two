...
async function scrapeSummitLagerImage() {
  try {
    console.log('Scraping Summit Lager image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/summit-lager';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('summit') || src.includes('lager') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'summit-lager.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Summit Lager%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Summit Lager not found in database.');
    }
...
scrapeSummitLagerImage()
  .then(() => {
    console.log('Summit Lager image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
