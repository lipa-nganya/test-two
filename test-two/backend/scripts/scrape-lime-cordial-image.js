...
async function scrapeLimeCordialImage() {
  try {
    console.log('Scraping Lime Cordial image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/lime-cordial';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('lime') || src.includes('cordial') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'lime-cordial.jpg';
    ...
    if (drink) {
      ...
      console.log('⚠️  Lime Cordial not found in database.');
    }
...
scrapeLimeCordialImage()
  .then(() => {
    console.log('Lime Cordial image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
