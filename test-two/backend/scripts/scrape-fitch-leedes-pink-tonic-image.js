...
async function scrapeFitchLeedesPinkTonicImage() {
  try {
    console.log('Scraping Fitch & Leedes Pink Tonic image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/fitch-leedes-pink-tonic';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('fitch') || src.includes('pink') || src.includes('tonic') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'fitch-leedes-pink-tonic.jpg';
    ...
    if (drink) {
      ...
      console.log('⚠️  Fitch & Leedes Pink Tonic not found in database.');
    }
...
scrapeFitchLeedesPinkTonicImage()
  .then(() => {
    console.log('Fitch & Leedes Pink Tonic image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
