...
async function scrapeStrettonDryGinImage() {
  try {
    console.log('Scraping Stretton Dry Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/stretton-dry-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('stretton') || src.includes('dry') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'stretton-dry-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Stretton Dry Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Stretton Dry Gin not found in database.');
    }
...
scrapeStrettonDryGinImage()
  .then(() => {
    console.log('Stretton Dry Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
