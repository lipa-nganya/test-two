...
async function scrapeTanquerayNoTenImage() {
  try {
    console.log('Scraping Tanqueray No. Ten image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/tanqueray-no-10-750ml';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tanqueray') || src.includes('no') || src.includes('ten') || src.includes('10') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'tanqueray-no-ten.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Tanqueray No. Ten%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Tanqueray No. Ten not found in database.');
    }
...
scrapeTanquerayNoTenImage()
  .then(() => {
    console.log('Tanqueray No. Ten image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
