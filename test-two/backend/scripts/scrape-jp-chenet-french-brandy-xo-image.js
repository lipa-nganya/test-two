...
async function scrapeJPChenetFrenchBrandyXOImage() {
  try {
    console.log('Scraping JP Chenet French Brandy XO image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/jp-chenet-french-brandy-xo';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('chenet') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'jp-chenet-french-brandy-xo.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%JP Chenet French Brandy XO%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  JP Chenet French Brandy XO not found in database.');
    }
...
scrapeJPChenetFrenchBrandyXOImage()
  .then(() => {
    console.log('JP Chenet French Brandy XO image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
