...
async function scrapeGreenallsBloodOrangeFigImage() {
  try {
    console.log('Scraping Greenalls Blood Orange & Fig image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/greenalls-blood-orange-fig';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('greenalls') || src.includes('blood') || src.includes('orange') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'greenalls-blood-orange-fig.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Greenalls Blood Orange & Fig%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Greenalls Blood Orange & Fig not found in database.');
    }
...
scrapeGreenallsBloodOrangeFigImage()
  .then(() => {
    console.log('Greenalls Blood Orange & Fig image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
