...
async function scrapeTanqueraySevillaImage() {
  try {
    console.log('Scraping Tanqueray Sevilla image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/tanqueray-sevilla';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('tanqueray') || src.includes('sevilla') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'tanqueray-sevilla.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        [db.Sequelize.Op.and]: [
          {
            name: {
              [db.Sequelize.Op.iLike]: '%Tanqueray Sevilla%'
            }
          },
          {
            name: {
              [db.Sequelize.Op.notILike]: '%Flor%'
            }
          }
        ]
      }
    ...
    if (drink) {
      ...
      console.log('⚠️  Tanqueray Sevilla not found in database.');
    }
...
scrapeTanqueraySevillaImage()
  .then(() => {
    console.log('Tanqueray Sevilla image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
