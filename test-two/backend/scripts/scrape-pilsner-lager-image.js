...
async function scrapePilsnerLagerImage() {
  try {
    console.log('Scraping Pilsner Lager image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/pilsner-lager';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('pilsner') || src.includes('lager') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'pilsner-lager.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Pilsner lager%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Pilsner lager not found in database.');
    }
...
scrapePilsnerLagerImage()
  .then(() => {
    console.log('Pilsner Lager image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
