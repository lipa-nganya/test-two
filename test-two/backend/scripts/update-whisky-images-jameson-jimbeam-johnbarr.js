...
async function getImageFromPage(pageUrl, drinkName, isFirstImage = false) {
  try {
    ...
    // For Jim Beam, get the first product image on the left
    if (isFirstImage) {
      // Look for the first product image in product cards/containers
      ...
      // Alternative: Look for first image with product context
      if (!imageUrl) {
        $('img[src*="cloudinary"][src*="product"]').not('[src*="logo"]').each((i, el) => {
          if (i === 0) {
...
async function updateWhiskyImages() {
  try {
    ...
    const itemsToUpdate = [
      { name: 'Jameson Whisky', url: 'https://www.dialadrinkkenya.com/jamesons', isFirstImage: false },
      { name: 'Jim Beam', url: 'https://www.dialadrinkkenya.com/jim-beam-bourbon-whisky', isFirstImage: true },
      { name: 'John Barr Red', url: 'https://www.dialadrinkkenya.com/search/john-barr-red', isFirstImage: false }
    ];
