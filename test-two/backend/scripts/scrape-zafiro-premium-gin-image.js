...
async function scrapeZafiroPremiumGinImage() {
  try {
    console.log('Scraping Zafiro Premium Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/zafiro-premium-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('zafiro') || src.includes('premium') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'zafiro-premium-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Zafiro Premium Gin%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Zafiro Premium Gin not found in database.');
    }
...
scrapeZafiroPremiumGinImage()
  .then(() => {
    console.log('Zafiro Premium Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
