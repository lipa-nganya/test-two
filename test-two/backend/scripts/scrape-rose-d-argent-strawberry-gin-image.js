...
async function scrapeRoseDArgentStrawberryGinImage() {
  try {
    console.log('Scraping Rose d\'Argent Strawberry Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/rose-d-argent-strawberry-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('rose') || src.includes('argent') || src.includes('strawberry') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'rose-d-argent-strawberry-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Rose d\'Argent%Strawberry%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Rose d\'Argent Strawberry Gin not found in database.');
    }
...
scrapeRoseDArgentStrawberryGinImage()
  .then(() => {
    console.log('Rose d\'Argent Strawberry Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
