...
async function scrapeStRemyVSOPImage() {
  try {
    console.log('Scraping St Remy VSOP image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/st-remy-vsop';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('st-remy') || src.includes('st.remy') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'st-remy-vsop.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%St Remy VSOP%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  St Remy VSOP not found in database.');
    }
...
scrapeStRemyVSOPImage()
  .then(() => {
    console.log('St Remy VSOP image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
