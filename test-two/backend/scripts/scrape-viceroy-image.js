...
async function scrapeViceroyImage() {
  try {
    console.log('Scraping Viceroy image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/viceroy-750ml';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('viceroy') || src.includes('brandy') || src.includes('product'))) {
          imageUrl = src;
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Viceroy%'
        },
        categoryId: {
          [db.Sequelize.Op.in]: await db.Category.findAll({ where: { name: 'Brandy' } }).then(cats => cats.map(c => c.id))
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Viceroy not found in database.');
    }
...
scrapeViceroyImage()
  .then(() => {
    console.log('Viceroy image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
