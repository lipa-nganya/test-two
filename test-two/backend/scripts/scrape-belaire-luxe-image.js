...
async function scrapeBelaireLuxeImage() {
  try {
    console.log('Scraping Belaire Luxe image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/belaire-luxe';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('belaire') || src.includes('champagne') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'belaire-luxe.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Belaire Luxe%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Belaire Luxe not found in database.');
    }
...
scrapeBelaireLuxeImage()
  .then(() => {
    console.log('Belaire Luxe image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
