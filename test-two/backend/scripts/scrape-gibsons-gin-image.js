...
async function scrapeGibsonsGinImage() {
  try {
    console.log('Scraping Gibson\'s Gin image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/gibson\'s-gin';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('gibson') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'gibsons-gin.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Gibson\'s Gin%'
        },
        name: {
          [db.Sequelize.Op.notILike]: '%Pink%'
        },
        name: {
          [db.Sequelize.Op.notILike]: '%Blood%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Gibson\'s Gin not found in database.');
    }
...
scrapeGibsonsGinImage()
  .then(() => {
    console.log('Gibson\'s Gin image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
