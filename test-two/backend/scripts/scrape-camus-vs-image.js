...
async function scrapeCamusVSImage() {
  try {
    console.log('Scraping Camus VS image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/camus-vs';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('camus') || src.includes('cognac') || src.includes('product'))) {
          imageUrl = src;
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Camus VS%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Camus VS not found in database.');
    }
...
scrapeCamusVSImage()
  .then(() => {
    console.log('Camus VS image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
