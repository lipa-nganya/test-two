...
async function scrapeStRemyXOImage() {
  try {
    console.log('Scraping St Remy XO image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search?query=St+Remy+XO';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%St Remy XO%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  St Remy XO not found in database.');
    }
...
scrapeStRemyXOImage()
  .then(() => {
    console.log('St Remy XO image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
