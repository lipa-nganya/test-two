...
async function scrapeHendricksMidsummerSolsticeImage() {
  try {
    console.log('Scraping Hendricks Midsummer Solstice image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/hendricks-midsummer-solstice';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('hendricks') || src.includes('midsummer') || src.includes('solstice') || src.includes('gin') || src.includes('product'))) {
          imageUrl = src;
    ...
    const filename = 'hendricks-midsummer-solstice.jpg';
    ...
    const drink = await db.Drink.findOne({
      where: {
        name: {
          [db.Sequelize.Op.iLike]: '%Hendricks Midsummer Solstice%'
        }
      }
    });
    ...
    if (drink) {
      ...
      console.log('⚠️  Hendricks Midsummer Solstice not found in database.');
    }
...
scrapeHendricksMidsummerSolsticeImage()
  .then(() => {
    console.log('Hendricks Midsummer Solstice image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
