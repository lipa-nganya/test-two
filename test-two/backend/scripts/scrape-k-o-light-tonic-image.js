...
async function scrapeKOLightTonicImage() {
  try {
    console.log('Scraping K.O Light Tonic image from Dial a Drink Kenya...');
    
    const url = 'https://www.dialadrinkkenya.com/search/k-o-light-tonic';
    ...
    if (!imageUrl) {
      ...
      allImages.each((i, img) => {
        ...
        if (src && (src.includes('k-o') || src.includes('light') || src.includes('tonic') || src.includes('product') || src.includes('beer'))) {
          imageUrl = src;
    ...
    const filename = 'k-o-light-tonic.jpg';
    ...
    if (drink) {
      ...
      console.log('⚠️  K.O Light Tonic not found in database.');
    }
...
scrapeKOLightTonicImage()
  .then(() => {
    console.log('K.O Light Tonic image scraping completed successfully');
    process.exit(0);
  })
  .catch((error) => {
